      ...
      <div className="relative h-96 w-96">
        {decks.map((deck, index) => (
          <div
            ...
            className="absolute left-1/2 top-1/2 w-48 h-72 transition-all duration-500 cursor-pointer"
            style={{
              transform:
                selectedCard === index
          ...
          >
            <div
              className="w-full h-full rounded-lg shadow-lg hover:shadow-xl transition-all duration-500 bg-white bg-opacity-60 flex flex-col justify-end items-center"
              style={{
            ...
            >
              {/* Show the image visually on the deck side */}
              <img src={deck} alt={`Deck ${index + 1}`} className="w-16 h-16 object-contain mb-2 border-2 border-primary bg-white rounded" />
            </div>
          </div>
        ))}
      </div>
